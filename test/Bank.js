...
const BankToken = artifacts.require("Bank");
...
contract("Bank Contract", (accounts) => {
    describe("Bank Smart Contract", function() {
        it("Create Bank", async function() {
            ...
            Bank = await BankToken.new(_name, _symbol, String(Rena.address), {from: creator})
        ...
        it("Approve allow Bank Transfer Rena From Account 0", async function () {
            ...
            let approveRena = await Rena.approve(String(Bank.address), amount, {from : creator});
        ...
        it("Check Balance of Account 0", async function() {
            let creator = accounts[0];
            expect(String(await Rena.balanceOf(String(creator)))).equal(String(30e18));
        })
        it("Check Balance Rena of Bank", async function() {
            expect(String(await Rena.balanceOf(String(Bank.address)))).equal(String(20e18));
        })

        it("Mint Rena for Bank", async function () {
            let mint = await Rena.mint(String(Bank.address), String(50e18), {from: accounts[0]});
            expect(String(await Rena.balanceOf(String(Bank.address)))).equal(String(70e18));
        })
