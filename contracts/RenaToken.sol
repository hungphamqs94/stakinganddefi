 ...
 * @title RenaToken
 */
contract RenaToken is ERC20, Ownable, Authorizable {
    ...
    function canUnlockAmount(address _holder) public view returns (uint256) {
        if (block.number < lockFromBlock) {
            ...
            uint256 numberLockBlock =
                lockToBlock - _lastUnlockBlock[_holder];
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "RenaToken::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "RenaToken::delegateBySig: invalid nonce"
        );
        require(block.timestamp <= expiry, "RenaToken::delegateBySig: signature expired");
        ...
        returns (uint256)
    {
        require(
            ...
            "RenaToken::getPriorVotes: not yet determined"
        );
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber =
            safe32(
                block.number,
                "RenaToken::_writeCheckpoint: block number exceeds 32 bits"
            );
