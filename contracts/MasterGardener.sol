...
import "./RenaToken.sol";
import "./Authorizable.sol";

// MasterGardener is the master gardener of whatever gardens are available.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once RENA is sufficiently
...
contract MasterGardener is Ownable, Authorizable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. RENA to distribute per block.
        uint256 lastRewardBlock; // Last block number that RENA distribution occurs.
        uint256 accGovTokenPerShare; // Accumulated RENA per share, times 1e12. See below.
    }

    // The RENA token
    RenaToken public govToken;
    ...
    // RENA created per block.
    uint256 public REWARD_PER_BLOCK;
    // Bonus multiplier for early RENA makers.
    ...
    // The block number when RENA mining starts.
    ...
    event PoolTransfer(address indexed token, address indexed sender);
    event Owner(address indexed own);
    ...
    constructor(
        ...
        for (uint256 i = 0; i < REWARD_MULTIPLIER.length - 1; i++) {
            uint256 halvingAtBlock = 1*(i+1)+10;
            HALVING_AT_BLOCK.push(halvingAtBlock);
        }
        FINISH_BONUS_AT_BLOCK = 1
            *(REWARD_MULTIPLIER.length - 1)
            .add(1);
        HALVING_AT_BLOCK.push(uint256(0));
    }

    function initValueAddr(
        address _devaddr,
    ...
    // Update the given pool's RENA allocation points. Can only be called by the owner.
    ...
    function updatePool(uint256 _pid) public {
        ...
        // Mint some new RENA tokens for the farmer and store them in MasterGardener.
    ...
    // View function to see pending RENA on frontend.
    ...
    // Deposit LP tokens to MasterGardener for RENA allocation.
    ...
    function onlyTestTransfer(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
